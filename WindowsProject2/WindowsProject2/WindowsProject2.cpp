...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow)
{
    ...
    ifstream in("D:\\Coding\\C++\\WindowsProject2\\123.txt");
    string buffer;
    int i = 0;
    if (in.is_open()) {
        while (!in.eof()) {
...
void DrawTable(HDC currDC, int windowWidth, int windowHeight) {
    ...
    int fontSize = 35; RECT rect = { 0, 0, 0, windowHeight };
    while (i < MAX_STRINGS) {
        rect.top = textHeight;
...
void InitDC(HWND hWnd, int windowWidth, int windowHeight) {
    ...
    HBRUSH hBkgrndBrush = CreateSolidBrush(RGB(194, 194, 194)); FillRect(currDC, &rect, hBkgrndBrush);
